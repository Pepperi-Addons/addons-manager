...
import {
  ...
  ViewChild, Output, ChangeDetectorRef, ElementRef, ComponentFactoryResolver, ViewContainerRef
} from '@angular/core';
...
import { PepAddonBlockLoaderService, PepRemoteLoaderService } from '@pepperi-addons/ngx-lib/remote-loader';
...
export class PepperiListContComponent {
        ...
        this.actions = {
            get: async (data: PepSelectionData) => {
                ...
                if (data?.rows.length === 1 && data.selectionType !== 0) {
                    ...
                    actions.push({
                        title: this.translate.instant('Addonmanager_Action_AuditLog'),
                        handler: async (params) => {
                            this.openAuditLogDialog(uuid);
                        }
                    });
    ...
    private setAddonsDataSource() {
        const searchObj: AddonsSearch = {
            ...
            SortBy: 'Name',
            ListType: 'addons',
            UUID: config.AddonUUID // this.addonData.Addon.UUID
        };
    ...
    private onHostEvents(event: any, dialogRef: MatDialogRef<any>) {
        if (dialogRef) {
    ...
    deletePermission(rowData) {
        ...
        const fieldsArr = this.existPermissions.filter(permission => permission.FieldID !== fieldID || permission.Title !== title);
